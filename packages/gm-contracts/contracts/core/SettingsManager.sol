...
import "./interfaces/ITierInfo.sol";
...
contract SettingsManager is ISettingsManager, Initializable, ReentrancyGuardUpgradeable, Constants {
    ...
    event SetTierInfo(address indexed tierInfo);
    ...
    event SetMaxTotalGm(uint256 maxTotalGm);
    ...
    uint256 public override maxTotalGm;
    ...
    function initialize(
        ...
        unused = 10000; // 10%
        maxTotalGm = 20 * 10 ** 6 * 10 ** GM_DECIMALS; // 20mil max gm supply
    }

    function initializeV2() public reinitializer(2) {
        bountyPercent_ = BountyPercent({firstCaller: 2000, resolver: 8000}); // first caller 2%, resolver 8% and leftover 90% to gm
    }
    ...
    function setTierInfo(address _tierInfo) external onlyOperator(3){
        tierInfo = ITierInfo(_tierInfo);
        emit SetTierInfo(_tierInfo);
    }
    ...
    function setMaxTotalGm(uint256 _maxTotalGm) external onlyOperator(3) {
        require(_maxTotalGm > 0, "invalid maxTotalGm");
        maxTotalGm = _maxTotalGm;
        emit SetMaxTotalGm(_maxTotalGm);
    }
    ...
    function getTierInfo(address _account) public override view returns (uint256) {
        return address(tierInfo) == address(0) ? BASIS_POINTS_DIVISOR : tierInfo.getTierInfo(_account);
    }
    ...
    function getTradingFee(
        ...
        (BASIS_POINTS_DIVISOR - deductFeePercent[_account]) *
            getTierInfo(_account)) / BASIS_POINTS_DIVISOR ** 2;
    }
    ...
    function getFundingRate(uint256 _tokenId) public view override returns (int256) {
        ...
        uint256 usdBalanceInVault = positionVault.getVaultUSDBalance();

        if (usdBalanceInVault == 0) return int256(0);
    ...
    function getMaxOpenInterestAmount(
        uint256 _tokenId,
        address _sender,
        bool _isLong
        ...
        uint256 _amount1 = _maxOpenInterestPerUser - openInterestPerUser[_sender];
        uint256 _amount2 = maxOpenInterestPerAssetPerSide[_tokenId][_isLong] - openInterestPerAssetPerSide[_tokenId][_isLong];
        uint256 _amount3 = maxTotalOpenInterest - totalOpenInterest;
        uint256 _minAmount12 = _amount1 < _amount2 ? _amount1 : _amount2;
        return _minAmount12 < _amount3 ? _minAmount12 : _amount3;
