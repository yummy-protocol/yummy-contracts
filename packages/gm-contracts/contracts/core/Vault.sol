...
import "../tokens/interfaces/IGUSD.sol";
import "../tokens/interfaces/IGM.sol";
...
contract Vault is Constants, Initializable, ReentrancyGuardUpgradeable, IVault {
    ...
    event Unstake(address indexed account, address token, uint256 gmAmount, uint256 amountOut);
    ...
    function initialize(address _operators, address _gm, address _gusd) public initializer {
    ...
    // deposit stablecoin to mint gusd
    function deposit(address _account, address _token, uint256 _amount) public nonReentrant preventBanners(msg.sender) {
        ...
        IGUSD(gusd).mint(_account, usdAmount - depositFee);
    ...
    // burn gusd to withdraw stablecoin
    function withdraw(address _token, uint256 _amount) public nonReentrant preventBanners(msg.sender) {
        ...
        IGUSD(gusd).burn(address(msg.sender), _amount);
    ...
    function withdrawAllUSDC() external {
        withdraw(address(USDC), IGUSD(gusd).balanceOf(msg.sender));
    }

    // stake stablecoin to mint gm
    function stake(address _account, address _token, uint256 _amount) public  nonReentrant preventBanners(msg.sender) returns (uint256) {
        ...
        uint256 mintAmount;
        uint256 totalGM = IERC20Upgradeable(gm).totalSupply();
        if (totalGM == 0) {
            mintAmount =
                (usdAmountAfterFee * DEFAULT_GM_PRICE * (10 ** GM_DECIMALS)) /
                (PRICE_PRECISION * BASIS_POINTS_DIVISOR);
        } else {
            mintAmount = (usdAmountAfterFee * totalGM) / totalUSD;
        }

        require(totalGM + mintAmount <= settingsManager.maxTotalGm(), "max total gm exceeded");

        _distributeFee(stakingFee, address(0), address(0));

        totalUSD += usdAmountAfterFee;
        lastStakedAt[_account] = block.timestamp;
        IMintable(gm).mint(_account, mintAmount);
    ...
    // burn gm to unstake stablecoin
    // gm cannot be unstaked or transferred within cooldown period, except whitelisted contracts
    function unstake(address _tokenOut, uint256 _gmAmount) public returns (uint256) {
        return unstake(msg.sender, _tokenOut, _gmAmount);
    }
    // burn gm to unstake stablecoin
    // gm cannot be unstaked or transferred within cooldown period, except whitelisted contracts
    function unstake(address _account, address _tokenOut, uint256 _gmAmount) public nonReentrant preventBanners(msg.sender) returns (uint256) {
        require(settingsManager.isUnstakingEnabled(_tokenOut), "unstaking disabled");
        uint256 totalGM = IERC20Upgradeable(gm).totalSupply();
        require(_gmAmount > 0 && _gmAmount <= totalGM, "gmAmount error");
        if (_account != msg.sender) require(settingsManager.checkDelegation(_account, msg.sender), "Not allowed");
        ...
        IMintable(gm).burn(_account, _gmAmount);

        uint256 usdAmount = (_gmAmount * totalUSD) / totalGM;
        ...
        emit Unstake(_account, _tokenOut, _gmAmount, tokenAmountOut);
        return tokenAmountOut;
    }

    function unstakeUSDC(uint256 _gmAmount) external {
        unstake(address(USDC), _gmAmount);
    }

    function unstakeAllUSDC() external {
        unstake(address(USDC), IERC20Upgradeable(gm).balanceOf(msg.sender));
    }
    ...
    function accountDeltaIntoTotalUSD(bool _isIncrease, uint256 _delta) external override onlyVault {
        if (_delta > 0) {
            if (_isIncrease) {
                ...
                require(totalUSD >= _delta, "exceeded GM bottom");
                totalUSD -= _delta;
            }
        }
    }
    ...
    function _distributeFee(uint256 _fee, address _refer, address _trader) internal {
        if (_fee > 0) {
            if (_refer != address(0)) {
                ...
                IGUSD(gusd).mint(_refer, referFee);
                if (traderRebate > 0) IGUSD(gusd).mint(_trader, traderRebate);
            ...
            if (_trader != address(0)) {
                ...
                if (platformFee > 0) {
                    ...
                    IGUSD(gusd).mint(platform, platformFee);
            ...
            uint256 feeForGM = (_fee * settingsManager.feeRewardBasisPoints()) / BASIS_POINTS_DIVISOR;
            totalUSD += feeForGM;
            IGUSD(gusd).mint(settingsManager.feeManager(), _fee - feeForGM);
        }
    }

    function takeGUSDIn(address _account, uint256 _amount) external override onlyVault {
        IGUSD(gusd).burn(_account, _amount);
    }

    function takeGUSDOut(address _account, uint256 _amount) external override onlyVault {
        IGUSD(gusd).mint(_account, _amount);
    }
    ...
    function forceClosePosition(uint256 _posId) external payable nonReentrant onlyOperator(1) {
        ...
        IGUSD(gusd).burn(position.owner, exceededPnl); // cap user's pnl to maxPnl
    ...
    function getGMPrice() external view returns (uint256) {
        uint256 totalGM = IERC20Upgradeable(gm).totalSupply();
        if (totalGM == 0) {
            return DEFAULT_GM_PRICE;
        } else {
            return (BASIS_POINTS_DIVISOR * (10 ** GM_DECIMALS) * totalUSD) / (totalGM * PRICE_PRECISION);
        }
    }
