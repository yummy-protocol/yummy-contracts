...
contract LiquidateVault is Constants, Initializable, ReentrancyGuardUpgradeable, ILiquidateVault {
    ...
    function liquidatePosition(uint256 _posId) external nonReentrant {
        ...
        uint256 gmBounty = position.collateral - firstCallerBounty - resolverBounty;

        if (liquidateRegistrant[_posId] == address(0)) {
            vault.takeGUSDOut(msg.sender, firstCallerBounty);
        } else {
            vault.takeGUSDOut(liquidateRegistrant[_posId], firstCallerBounty);
        }
        vault.takeGUSDOut(msg.sender, resolverBounty);
        vault.accountDeltaIntoTotalUSD(true, gmBounty);
