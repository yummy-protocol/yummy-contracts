...
import "./interfaces/IGmyIou.sol";
import "./interfaces/IAmmRouter.sol";
import "./interfaces/IGmyMigrator.sol";

contract GmyMigrator is ReentrancyGuard, IGmyMigrator {
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin, "GmyMigrator: forbidden");
        _;
    }

    modifier onlySigner() {
        require(isSigner[msg.sender], "GmyMigrator: forbidden");
        _;
    }
    ...
    function initialize(
        ...
        require(!isInitialized, "GmyMigrator: already initialized");
        require(_whitelistedTokens.length == _iouTokens.length, "GmyMigrator: invalid _iouTokens.length");
        require(_whitelistedTokens.length == _prices.length, "GmyMigrator: invalid _prices.length");
        require(_whitelistedTokens.length == _caps.length, "GmyMigrator: invalid _caps.length");
        require(_lpTokens.length == _lpTokenAs.length, "GmyMigrator: invalid _lpTokenAs.length");
        require(_lpTokens.length == _lpTokenBs.length, "GmyMigrator: invalid _lpTokenBs.length");
    ...
    function migrate(
        ...
        require(isMigrationActive, "GmyMigrator: migration is no longer active");
        require(whitelistedTokens[_token], "GmyMigrator: token not whitelisted");
        require(_tokenAmount > 0, "GmyMigrator: invalid tokenAmount");

        if (hasMaxMigrationLimit) {
            migratedAmounts[msg.sender][_token] = migratedAmounts[msg.sender][_token].add(_tokenAmount);
            require(migratedAmounts[msg.sender][_token] <= maxMigrationAmounts[msg.sender][_token], "GmyMigrator: maxMigrationAmount exceeded");
        }

        uint256 tokenPrice = getTokenPrice(_token);
        uint256 mintAmount = _tokenAmount.mul(tokenPrice).div(gmyPrice);
        require(mintAmount > 0, "GmyMigrator: invalid mintAmount");

        tokenAmounts[_token] = tokenAmounts[_token].add(_tokenAmount);
        require(tokenAmounts[_token] < caps[_token], "GmyMigrator: token cap exceeded");
        ...
        if (lpTokens[_token]) {
            ...
            require(tokenA != address(0), "GmyMigrator: invalid tokenA");
            require(tokenB != address(0), "GmyMigrator: invalid tokenB");
        ...
        IGmyIou(iouToken).mint(msg.sender, mintAmount);
    }
    ...
    function signApprove(address _token, address _spender, uint256 _amount, uint256 _nonce) external nonReentrant onlySigner {
        ...
        require(!signedActions[msg.sender][action], "GmyMigrator: already signed");
    ...
    function getTokenPrice(address _token) public view returns (uint256) {
        ...
        require(price != 0, "GmyMigrator: invalid token price");
        return price;
    }

    function getIouToken(address _token) public view returns (address) {
        address iouToken = iouTokens[_token];
        require(iouToken != address(0), "GmyMigrator: invalid iou token");
        return iouToken;
    }
    ...
    function _validateAction(bytes32 _action) private view {
        require(pendingActions[_action], "GmyMigrator: action not signalled");
    }
    ...
    function _validateAuthorization(bytes32 _action) private view {
        ...
        if (count == 0) {
            revert("GmyMigrator: action not authorized");
        }
        require(count >= minAuthorizations, "GmyMigrator: insufficient authorization");
    }

    function _clearAction(bytes32 _action, uint256 _nonce) private {
        require(pendingActions[_action], "GmyMigrator: invalid _action");
