...
contract Treasury is ReentrancyGuard, ITimelockTarget {
    ...
    uint256 public buSDGasisPoints;
    ...
    function swap(uint256 _busdAmount) external nonReentrant {
        ...
        // receive BUSD
        uint256 buSDGefore = IERC20(busd).balanceOf(address(this));
        IERC20(busd).transferFrom(account, address(this), _busdAmount);
        uint256 busdAfter = IERC20(busd).balanceOf(address(this));
        require(busdAfter.sub(buSDGefore) == _busdAmount, "Treasury: invalid transfer");
    ...
    function addLiquidity() external onlyGov nonReentrant {
        ...
        uint256 busdAmount = busdReceived.mul(buSDGasisPoints).div(BASIS_POINTS_DIVISOR);
    ...
    function increaseBuSDGasisPoints(uint256 _buSDGasisPoints) external onlyGov nonReentrant {
        require(_buSDGasisPoints > buSDGasisPoints, "Treasury: invalid _buSDGasisPoints");
        buSDGasisPoints = _buSDGasisPoints;
    }
