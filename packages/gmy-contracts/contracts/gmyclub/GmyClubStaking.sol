...
contract GmyClubStaking is IERC721Receiver, Ownable, ReentrancyGuard
{
    ...
    function _stake(address _account, uint256 _tokenId) internal virtual {
        uint256 _power = uint256(IGmyClubNFT(nft).getTokenPower(_tokenId));
    ...
    function allocateRewardManually(uint256 _amount) public nonReentrant checkEpoch onlyKeeper
    {
        require(_amount > 0, "Cannot allocate 0");
        require(totalPower > 0, "Cannot allocate when totalSupply is 0");
        epochReward = _amount;

        // Create & add new snapshot
        uint256 prevRPS = getLatestSnapshot().rewardPerShare;
        uint256 nextRPS = prevRPS + ((_amount * 1e18) / totalPower);
