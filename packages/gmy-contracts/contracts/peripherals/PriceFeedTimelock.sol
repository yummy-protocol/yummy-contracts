...
contract PriceFeedTimelock {
    ...
    event SignalSetTokens(
        address _fastPriceFeed,
        address[] _tokens,
        uint256[] _tokenPrecisions
    );
    ...
    function setPythNetwork(address _priceFeed, address _pythNetwork) external onlyAdmin {
        IVaultPriceFeed(_priceFeed).setPythNetwork(_pythNetwork);
    }
    ...
    function setTokens(
        ...
        IFastPriceFeed(_fastPriceFeed).setTokens(
            _tokens,
            _tokenPrecisions
        );
    }
