...
contract EsGmyBatchSender {
    using SafeMath for uint256;

    address public admin;
    address public esGmy;

    constructor(address _esGmy) public {
        admin = msg.sender;
        esGmy = _esGmy;
    }

    modifier onlyAdmin() {
        require(msg.sender == admin, "EsGmyBatchSender: forbidden");
        _;
    }
    ...
    function send(
        ...
        for (uint256 i = 0; i < _accounts.length; i++) {
            IERC20(esGmy).transferFrom(msg.sender, _accounts[i], _amounts[i]);
